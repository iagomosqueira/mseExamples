# data_ang.R - DESC
# /home/mosquia/Active/WORKSHOP_JRC_MSE-MAY2022/data_ang.R

# Copyright (c) WUR, 2022.
# Author: Iago MOSQUEIRA (WMR) <iago.mosqueira@wur.nl>
#
# Distributed under the terms of the EUPL-1.2


library(mse)
library(ss3om)

# SET final year & iters

fy <- 2040
nit <- 10

# LOAD anglerfish SS3 run

ang <- readOutputss3('bootstrap/data/ang/')

# --- BUILD om

# BUILD objects

inp <- buildFLBFss330(ang)

# CONSTRUCT conditioned om

com <- FLombf(biols=FLBiols(A=inp$bio), fisheries=inp$fisheries,
  projection=mseCtrl(method=fwd.om))

plot(com)

# EXTEND and propagate

om <- propagate(fwdWindow(com, end=fy), nit)

# FUTURE SRR deviances

sr(biol(om))$residuals <- window(sr(biol(om))$residuals, end=2040)

sr(biol(om))$residuals <- expand(
  ar1rlnorm(rho=0.3, 1950:2040, iters=nit, sdlog=0.6), unit=c('F', 'M'))

# ADD refpts

refpts(om, 1) <- buildFLRPss330(ang)




# --- BUILD oem

# LOAD stk

stk <- fwdWindow(readFLSss3('bootstrap/data/ang/', wtatage=TRUE), end=2040)
range(stk, c("minfbar", "maxfbar")) <- c(3, 15)

# BUG:

for(i in 1:4) {
  landings.wt(fisheries(om)[[i]][[1]]) <- landings.wt(stk)
  discards.wt(fisheries(om)[[i]][[1]]) <- discards.wt(stk)
}

# SIMPLIFY FLStock, no sexes

stk <- propagate(simplify(stk, 'unit'), nit)

# CREATE two surveys

idx <- buildFLIBss330(ang)

# indices

sur <- FLIndices(
  Q1=FLIndex(
    sel.pattern=sel.pattern(idx[[1]])[,,1],
    range=c(startf=0.15, endf=0.15)),
  Q3=FLIndex(
    sel.pattern=sel.pattern(idx[[2]])[,,1],
    range=c(startf=0.75, endf=0.75))
)

dimnames(sur[[1]])<-list(unit='unique', season='all')
dimnames(sur[[2]])<-list(unit='unique', season='all')

fit <- FLQuants(
  Q1=(stock.n(stk) * exp(-z(stk) * 0.15))[, ac(2003:2021)] *
    unitMeans(sel.pattern(idx[[1]])),
  Q3=(stock.n(stk) * exp(-z(stk) * 0.75))[, ac(2006:2021)] *
    unitMeans(sel.pattern(idx[[2]])))

iqs <- computeQ(sur, stk, fit=fit)

# BUG: 2021
iqs[[1]][,'2021'] <- iqs[[1]][,'2020']
iqs[[2]][,'2021'] <- iqs[[2]][,'2020']

index.q(sur[[1]]) <- iqs[[1]]
index.q(sur[[2]]) <- iqs[[2]]

sur[[1]] <- survey(stk, sur[[1]])
sur[[2]] <- survey(stk, sur[[2]])

sur <- fwdWindow(sur, end=2040)

sur <- FLIndices(Map(function(x, y){
  x@index = x@index * rlnorm(nit, x@index%=%0, y)
  x
  }, x=sur, y=c(0.23,0.27)) )

# OEM

oem <- FLoem(method=sampling.oem,
  observations=list(A=list(stk=stk, idx=sur)),
  deviances=list(A=list(
    stk=FLQuants(
      catch.n=rlnorm(nit, catch.n(stk) %=% 0, 0.2)),
    idx=FLQuants(Q1=rlnorm(nit, index.q(sur[[1]]) %=% 0, 0.1),
      Q2=rlnorm(nit, index.q(sur[[2]]) %=% 0, 0.1)))))

# SAVE

save(om, oem, file="data/om.Rdata", compress="xz")
